...
func TestDeleteLimits(t *testing.T) {
	testTable := []struct {
	...
	}{
		{
			...
			mockRepo: func(controller *gomock.Controller) repository.LimitsRepository {
				...
				mock.EXPECT().DeleteClientLimits(gomock.Any(), "test-client-id").Return(nil)
				return mock
			},
			expectedError: nil,
		},
		...
		},
		{
			...
			mockRepo: func(controller *gomock.Controller) repository.LimitsRepository {
				mock := mock_repository.NewMockLimitsRepository(controller)
				mock.EXPECT().IsClientExists(gomock.Any(), "test-client-id").Return(true, nil)
				mock.EXPECT().DeleteClientLimits(gomock.Any(), "test-client-id").Return(model.ErrDb)
				return mock
			},
			expectedError: model.ErrDb,
		},
	...
	for _, testCase := range testTable {
		t.Run(testCase.name, func(t *testing.T) {
			...
			err := limitsManager.DeleteClientLimits(ctx, "test-client-id")
